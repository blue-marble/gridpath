...
class Project:
    def __init__(self,name,operational_type):
        ...
        if operational_type == 'gen_var' or operational_type == 'gen_var_must_take':
            ...
            self.input_filenames = ['variable_generator_profiles','project_availability_exogenous']
            self.input_headers = [['project','timepoint','cap_factor'],['project','timepoint','availability_derate','hyb_stor_cap_availability_derate']]
        
        elif operational_type == 'gen_hydro_must_take':
            self.input_filenames = ['hydro_conventional_horizon_params']
            self.input_headers = [['project','horizon','average_power_fraction','min_power_fraction','max_power_fraction']]
        
        else:
            self.input_filenames = ['project_availability_exogenous']
            self.input_headers = [['project','timepoint','availability_derate','hyb_stor_cap_availability_derate']]

class Unit:
    def __init__(self,unit_name,project_name,unit_type,unit_mw,unit_timeseries,N_units,unitModel,unitFOR,unitMTTR,gentieModel,gentieFOR,gentieMTTR):
...
def calculate_timepoint(weather_mode,iteration,hour_of_year,hydro_year=0,weather_year=0):

    if weather_mode == 'MonteCarlo':
        timepoint = str(iteration)+str(hour_of_year).zfill(4)
    elif weather_mode == 'Synchronized':
        timepoint = str(iteration)+str(hydro_year)+str(weather_year)+str(hour_of_year).zfill(4)

    return timepoint

def write_temporal_files(subp,subp_of_year,case_name,weather_mode,opt_window,hydro_balancing_window,subproblem_balancing_type,study_year,subp_per_year,iteration,hydro_year=0,weather_year=0):
        
    # print periods file
    with open(os.path.join('Simulations',case_name,str(subp),'inputs','periods.tab'),'w',newline='') as csvfile:
        csvwriter = csv.writer(csvfile, delimiter = '\t')
        csvwriter.writerow(['period','discount_factor','period_start_year','period_end_year','hours_in_period_timepoints'])
        csvwriter.writerow([study_year,'1',study_year,study_year+1,opt_window*subp_per_year*24.0])
    
    # print horizons file
    with open(os.path.join('Simulations',case_name,str(subp),'inputs','horizons.tab'),'w',newline='') as csvfile:
        csvwriter = csv.writer(csvfile, delimiter = '\t')
    ...
    horizon_timepoints_file = open(os.path.join('Simulations',case_name,str(subp),'inputs','horizon_timepoints.tab'),'w',newline='')
    ...
    timepoint_file = open(os.path.join('Simulations',case_name,str(subp),'inputs','timepoints.tab'),'w',newline='')
    ...
    for t in range(opt_window*24):
        ...
        timepoint = calculate_timepoint(weather_mode,iteration,hour_of_year,hydro_year,weather_year)
        
        if weather_mode == 'MonteCarlo':
            month = (np.datetime64(str(study_year)+'-01-01') + np.timedelta64(hour_of_year-1,'h')).astype(object).month
        elif weather_mode == 'Synchronized':
            month = (np.datetime64(str(weather_year)+'-01-01') + np.timedelta64(hour_of_year-1,'h')).astype(object).month
...
def simulate_outages(outage_model,FOR,MTTR,N_units,dt,starting_outage_states=[]):
    ...
    if outage_model == 'Derate':
        ...
        availability = 1.0-(np.random.rand(len(FOR),N_units) < np.outer(FOR,np.ones(N_units)))
        ...
        if np.size(starting_outage_states) == 0:
            avail_last = 1.0-(np.random.rand(1,N_units) < np.outer(FOR[0],np.ones(N_units)))
        else:
        ...
        for t in range(len(FOR)):
            avail_tmp = (avail_last == 1)*(1.0 - (np.random.exponential(MTTF[t],N_units) < dt)) + (avail_last == 0)*(np.random.exponential(float(MTTR),N_units) < dt)
...
def write_load_inputs(load_zone,case_name,study_year,loads,load_names,timeseries,timeseries_names,weather_mode,iterations,opt_window,load_no):
    ...
    for load_name in load_zone.load_list:
        ...
        timeseries_data.append(pd.read_csv(os.path.join('timeseries_data',ts.name,load.name+'.csv')))
        ...
        for j in range(len(hydro_years)):
            for k in range(len(weather_years)):
                ...
                for subp_of_year in range(N_subp_per_year):
                    ...
                    load_zone_temp_file = open(os.path.join('Simulations',case_name,str(subp),'inputs',load_zone.input_filename,load_zone.name+'.csv'),'w',newline='')
                    ...
                    for d in range(opt_window):
                        ...
                        for l in range(len(load_zone.load_list)):
                            ...
                            load_mw += day_timeseries_data['load_mw'].to_numpy(dtype=float)
                        
                        # print load data to the subproblem temporary file
                        for t in range(24):
                            hour_of_year = subp_of_year*opt_window*24+d*24+t+1
                            timepoint = calculate_timepoint(weather_mode,i+1,hour_of_year,hydro_years[j],weather_years[k])
                            load_zone_writer.writerow([load_zone.name,timepoint,np.round(load_mw[t],0)])
                        
                        draw_day += 1
                    
                    load_zone_temp_file.close()    

def write_project_inputs(project,case_name,study_year,units,unit_names,timeseries,timeseries_names,weather_mode,iterations,opt_window,hydro_balancing_window,project_no):
    ...
    for unit_name in project.unit_list:
        ...
        if unit.timeseries != 'NA':
            ...
            ts = timeseries[ts_ind]
            timeseries_data.append(pd.read_csv(os.path.join('timeseries_data',ts.name,unit.name+'.csv')))
            ...
            for k in range(len(weather_years)):
                ...
                for subp_of_year in range(N_subp_per_year):
                    ...
                    for f in range(len(project.input_filenames)):
                        project_temp_files.append(open(os.path.join('Simulations',case_name,str(subp),'inputs',project.input_filenames[f],project.name+'.csv'),'w',newline=''))
                    ...
                    for d in range(opt_window):
                        ...
                        if project.operational_type == 'gen_hydro_must_take':
                            if np.mod(d,hydro_balancing_window) == 0:
                                project_data = pd.DataFrame(0, index=np.arange(1), columns=['avg_hydro','min_hydro','max_hydro'])
                        
                        elif project.operational_type == 'gen_var' or project.operational_type == 'gen_var_must_take':
                            project_data = pd.DataFrame(0, index=np.arange(24), columns=['cap_factor'])
                        
                        elif project.operational_type == 'gen_var_stor_hyb':
                            project_data = pd.DataFrame(0, index=np.arange(24), columns=['cap_factor','hyb_stor_cap_availability_derate'])

                        else:    
                            project_data = pd.DataFrame(0, index=np.arange(24), columns=['availability_derate'])
                        ...
                        for u in range(len(project.unit_list)):
                            ...
                            if unit.timeseries != 'NA':
                                ...
                                # pull the timeseries data for the day
                                day_timeseries_data = timeseries_data[u].loc[ts_indices,:]
                            else:
                            ...
                            if 'max_output' in day_timeseries_data.columns:
                                p_maxoutput = day_timeseries_data['max_output'].to_numpy(dtype=float)
                            else:
                                p_maxoutput = np.ones(T)
                            
                            # pull unit FOR data, if available
                            if 'unitFOR' in day_timeseries_data.columns:
                                p_unitFOR = day_timeseries_data['unitFOR'].to_numpy(dtype=float)
                            else:
                                if unit.unitFOR != 'NA':
                            ...
                            if 'gentieFOR' in day_timeseries_data.columns:
                                p_gentieFOR = day_timeseries_data['unitFOR'].to_numpy(dtype=float)
                            else:
                            ...
                            if i == 0:
                                unit_avail = simulate_outages('Derate',p_unitFOR,unit.unitMTTR,unit.units,dt)
                                gentie_avail = simulate_outages('Derate',p_gentieFOR,unit.gentieMTTR,1,dt)
                            ...
                            # for all other iterations, use the outage model specified for the unit
                                ...
                                if draw_day == 0:
                                    unit_avail = simulate_outages(unit.unitModel,p_unitFOR,unit.unitMTTR,unit.units,dt)
                                    gentie_avail = simulate_outages(unit.gentieModel,p_gentieFOR,unit.gentieMTTR,1,dt)
                                # otherwise, provide the outage states for the unit from the prior timestep
                                else:
                                    unit_avail = simulate_outages(unit.unitModel,p_unitFOR,unit.unitMTTR,unit.units,dt,unit.unit_avail_last)
                                    gentie_avail = simulate_outages(unit.gentieModel,p_gentieFOR,unit.gentieMTTR,1,dt,unit.gentie_avail_last)
                            ...
                            outage_adjustment = np.mean(unit_avail,axis=1)*np.mean(gentie_avail,axis=1)
                            
                            # incorporate unit availability into weighted average project availability
                            if project.operational_type == 'gen_hydro_must_take':  
                                project_data += day_timeseries_data[['avg_hydro','min_hydro','max_hydro']].to_numpy()*outage_adjustment[0]*unit.mw/project.total_mw/hydro_balancing_window
  
                            elif project.operational_type == 'gen_var' or project.operational_type == 'gen_var_must_take':
                                project_data['cap_factor'] += p_maxoutput*outage_adjustment*unit.mw/project.total_mw
  
                            elif project.operational_type == 'gen_var_stor_hyb': 
                                if unit.unit_type == 'var_gen':
                                    project_data['cap_factor'] += p_maxoutput*outage_adjustment*unit.mw/project.total_var_mw
                                elif unit.unit_type == 'stor':
                                    project_data['hyb_stor_cap_availability_derate'] += p_maxoutput*outage_adjustment*unit.mw/project.total_stor_mw
                                    
                            else:
                                project_data['availability_derate'] += p_maxoutput*outage_adjustment*unit.mw/project.total_mw
                                
                                
                        # print project availability data to the subproblem temporary files
                        for f in range(len(project_temp_files)):
                            
                            if project.input_filenames[f] == 'hydro_conventional_horizon_params': 
                                # only print hydro data if it's the last day of a hydro horizon
                                if np.mod(d,hydro_balancing_window) == hydro_balancing_window-1:
                                    hydro_horizon += 1
                                    project_data = np.round(project_data,N_round)
                                    project_writers[f].writerow([project.name,hydro_horizon,project_data.loc[0,'avg_hydro'],project_data.loc[0,'min_hydro'],project_data.loc[0,'max_hydro']])
                                
                            elif project.input_filenames[f] == 'variable_generator_profiles':
                                for t in range(24):
                                    hour_of_year = subp_of_year*opt_window*24+d*24+t+1
                                    timepoint = calculate_timepoint(weather_mode,i+1,hour_of_year,hydro_years[j],weather_years[k])
                                    project_data = np.round(project_data,N_round)
                                    project_writers[f].writerow([project.name,timepoint,project_data.loc[t,'cap_factor']])
                                ...
                                for t in range(24):
                                    ...
                                    timepoint = calculate_timepoint(weather_mode,i+1,hour_of_year,hydro_years[j],weather_years[k])
                                    if project.operational_type == 'gen_var_stor_hyb':
                                        project_data['hyb_stor_cap_availability_derate'] = np.round(project_data['hyb_stor_cap_availability_derate'],N_round_stor)
                                        project_writers[f].writerow([project.name,timepoint,'.',project_data.loc[t,'hyb_stor_cap_availability_derate']])
                                    else:
                                        project_data['availability_derate'] = np.round(project_data['availability_derate'],N_round)
                                        project_writers[f].writerow([project.name,timepoint,project_data.loc[t,'availability_derate'],'.'])
...
def consolidate_files(case_name,subp):
    ...
    input_files = os.listdir(os.path.join('Simulations',case_name,str(subp),'inputs'))
    
    for input_file in input_files:
    
        with open(os.path.join('Simulations',case_name,str(subp),'inputs',input_file+'.tab'),'w',newline='') as final_file:
            input_writer = csv.writer(final_file,delimiter = '\t')
        
            # loop through the output files
            i = 0
            for temporary_file in os.listdir(os.path.join('Simulations',case_name,str(subp),'inputs',input_file)):
                with open(os.path.join('Simulations',case_name,str(subp),'inputs',input_file,temporary_file)) as temp_file:
        ...
        shutil.rmtree(os.path.join('Simulations',case_name,str(subp),'inputs',input_file))
...
if __name__ == '__main__': 
    ...
    load_zone_names = []
    load_zones = []
    if os.path.exists(os.path.join('common_files',common_files,'subproblems','load_zones.tab')):
        with open(os.path.join('common_files',common_files,'subproblems','load_zones.tab')) as csvfile:
        ...
        with open(os.path.join('settings',loads_file)) as csvfile:
            ...
            for row in file_reader:
                ...
                if load_zone_name in load_zone_names:
                    ...
                    loads.append(Load(load_name,load_zone_name,load_timeseries,load_scalar))
    ...
    project_names = []
    projects = []
    if os.path.exists(os.path.join('common_files',common_files,'subproblems','projects.tab')):
        with open(os.path.join('common_files',common_files,'subproblems','projects.tab')) as csvfile:
            file_reader = csv.reader(csvfile,delimiter='\t')
            file_reader.__next__()
            for row in file_reader:
                name = row[0]
        ...
        with open(os.path.join('settings',units_file)) as csvfile:
            ...
            for row in file_reader:
                ...
                if project_name in project_names:
                    ...
                    units.append(Unit(unit_name,project_name,unit_type,unit_mw,unit_timeseries,N_units,unitModel,unitFOR,unitMTTR,gentieModel,gentieFOR,gentieMTTR))
    ...
    for ts in timeseries:
        ...
        timestamps_tmp = pd.read_csv(os.path.join('timeseries_data',ts.name,'timestamps.csv'))
            ...
            if ts.stat == 'hyd':
                ...
                for hydro_bin in hydro_bins.index:
                    ...
                    if year in timestamp_years:
                        ...
                        mask = (ts.timestamps['year'] == year) & (ts.timestamps['month'] == month)
                ...
                for met_bin in weather_bins.index:
                    ...
                    if year in timestamp_years:
                        ...
                        mask = (ts.timestamps['year'] == year) & (ts.timestamps['month'] == month) & (ts.timestamps['day'] == day)
                        ts.met_bins[mask] = weather_bins.loc[met_bin,'weather_bin']
                        if ts.stat == 'cmb':
                            ts.daytype_bins[mask] = weather_bins.loc[met_bin,'daytype_bin']
            ...
            for k in range(len(weather_years)):
                for subp_of_year in range(N_subp_per_year):
                    ...
                    for load_zone in load_zones:
                        ...
                        if os.path.exists(os.path.join('Simulations',case_name,str(subp),'inputs',input_filename)) == False:
                            os.makedirs(os.path.join('Simulations',case_name,str(subp),'inputs',input_filename))
    ...
    if weather_mode == 'MonteCarlo':
        ...
        for yr in range(iterations):
            ...
            hydrobin_tmp = hydro_bins['hydro_bin'][np.random.randint(hydro_bins.index.size)]
            ...
            starting_met_options = weather_bins[weather_bins['month'] == 1]['weather_bin'].to_numpy()
            simmetbin = starting_met_options[np.random.randint(len(starting_met_options))]
            ...
            for subp in range(N_subp_per_year):
                ...
                for d in range(opt_window):
                    ...
                    simdaytype = (simdate.astype(datetime.datetime).isoweekday() > 5)*1
                    ...
                    if i > 0:
                        ...
                        met_options_tmp = weather_bins[weather_bins['month'] == simmonth]['weather_bin'].to_numpy()
                        # find the indices where the weather bin matches the prior weather bin, and add 1
                        nextbin_inds = np.array(np.where(met_options_tmp == prior_metbin))+1
                        ...
                        simmetbin = nextbin_options[np.random.randint(len(nextbin_options))]
                        prior_metbin = simmetbin
                    
                    # prepare draw information to print
                    draw_data = np.array([N_subp_per_year*yr+subp+1,d+1,hydrobin_tmp,simmonth,simmetbin,simdaytype])
                    ...
                    for ts in timeseries:
                        ...
                        if ts.stat == 'hyd':
                            bin_mask = (ts.timestamps['month'] == simmonth) & (ts.hydro_bins == hydrobin_tmp)
                        
                        # if meteorological model, find intervals with matching months and weather
                        if ts.stat == 'met':
                            bin_mask = (ts.timestamps['month'] == simmonth) & (ts.met_bins == simmetbin)
                        
                        # if combined meteorological and daytype model, find intervals with matching months, weather, and day-types
                        if ts.stat == 'cmb':
                            bin_mask = (ts.timestamps['month'] == simmonth) & (ts.met_bins == simmetbin) & (ts.daytype_bins == simdaytype)
        ...
        # print the header for the draw information
        header = ['subproblem','day','hydro year','weather year','weather month','weather day']
        for ts in timeseries:
            for h in ts.timestamps.columns:
                header.append(ts.name+' '+h)
        draw_data_writer.writerow(header)
        ...
        for i in range(len(hydro_years)):
            ...
            for j in range(len(weather_years)):
                ...
                weatherdate = np.datetime64(str(starting_weatheryear)+'-01-01') - np.timedelta64(1,'D')
                ...
                for subp in range(N_subp_per_year):
                    ...
                    for d in range(opt_window):
                        ...
                        weather_timestamp = [weatheryear,weathermonth,weatherdayofmonth]
                        
                        # if in a new weather year, update the hydro year to the next year, or loop around if needed
                        # NOTE THIS WILL BE HELPFUL IF NOT STARTING ON JAN 1
                        hydroyear = starting_hydroyear + weatheryear - starting_weatheryear
                        ...
                        # print draw data
                        draw_data_writer.writerow([subp+1,d+1,hydroyear,weatheryear,weathermonth,weatherdayofmonth])
                        
                        # find the indices corresponding to the draw in each timeseries
                        for ts in timeseries:
                            # if hydro statistical model, find intervals with matching hydro years and months
                            if ts.stat == 'hyd':
                                ts.draw_indices.append((ts.timestamps == hydro_timestamp).all(1))
                            # if meteorological or combined meteorological/daytype model, find intervals with matching weather year, month, and day
                                ...
                                ts.draw_indices.append((ts.timestamps == weather_timestamp).all(1))
    ...
    i = 0
    for load_zone in load_zones:
        write_load_inputs(load_zone,case_name,study_year,loads,load_names,timeseries,timeseries_names,weather_mode,iterations,opt_window,i)
        i += 1
   
    print('simulating project availability...')
    i = 0
    for project in projects:
        write_project_inputs(project,case_name,study_year,units,unit_names,timeseries,timeseries_names,weather_mode,iterations,opt_window,hydro_balancing_window,i)
        i += 1
            ...
            for k in range(len(weather_years)):
                for subp_of_year in range(N_subp_per_year):
                    ...
                    for file in os.listdir(subproblem_dir):
                        shutil.copy(os.path.join(subproblem_dir,file),os.path.join('Simulations',case_name,str(subp),'inputs',file))
                    
                    # print temporal files - NOTE: typically parallelize this
                    if weather_mode == 'MonteCarlo':
                        write_temporal_files(subp,subp_of_year+1,case_name,weather_mode,opt_window,hydro_balancing_window,subproblem_balancing_type,study_year,N_subp_per_year,i+1)
                    elif weather_mode == 'Synchronized':
                        write_temporal_files(subp,subp_of_year+1,case_name,weather_mode,opt_window,hydro_balancing_window,subproblem_balancing_type,study_year,N_subp_per_year,i+1,hydro_years[j],weather_years[k])
