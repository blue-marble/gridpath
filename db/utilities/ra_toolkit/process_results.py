...
draw_data = pd.read_csv(os.path.join('Simulations',case_name+'_log','draw_data.csv'))
...
weight = np.zeros(12)
days_in_study_year = (np.datetime64(str(study_year+1)+'-01-01')-np.datetime64(str(study_year)+'-01-01'))/np.timedelta64(1,'D')
for month in range(1,13): 
    if month < 12:
        studyyear_days_per_month = (np.datetime64(str(study_year)+'-'+str(month+1).zfill(2)+'-01')-np.datetime64(str(study_year)+'-'+str(month).zfill(2)+'-01'))/np.timedelta64(1,'D')
    else:
        studyyear_days_per_month = 31
    if weather_mode == 'MonteCarlo':
        sim_days_per_month = draw_data[draw_data['month'] == month].index.size/N_years
    elif weather_mode == 'Synchronized':
        sim_days_per_month = draw_data[draw_data['weather month'] == month].index.size/(len(hydro_years)*len(weather_years))
    ...
    for j in range(len(hydro_years)):
        for k in range(len(weather_years)):
            ...
            for subp_of_year in range(N_subp_per_year):
                ...
                if os.path.exists(os.path.join('Simulations',case_name,str(subp),'results','load_balance.csv')):
                    load_balance = pd.read_csv(os.path.join('Simulations',case_name,str(subp),'results','load_balance.csv')) 
                    subp_use = load_balance.pivot(index='timepoint',columns='zone',values='unserved_energy_mw')
                    ...
                    if subp_use.index.size > 0:
                        ...
                        day_of_subp = np.floor((subp_timepoints - np.min(subp_timepoints))/24).astype(int)+1
                        subp_use['subproblem'] = subp
                        subp_use['simulation_day'] = (subp-1)*opt_window + day_of_subp
                        subp_use['iteration'] = i+1
                        subp_use['HE'] = np.mod(hour_of_year-1,24)+1
                        
                        if weather_mode == 'MonteCarlo':
                            timestamp = np.datetime64(str(study_year)+'-01-01') + (hour_of_year-1)*np.timedelta64(1,'h')
                            month = timestamp.astype('datetime64[M]').astype(int) % 12 + 1
                            subp_use['month'] = month
                            subp_use['hydro_year'] = draw_data.loc[(draw_data['subproblem'] == subp),'hydro year'].to_numpy()[0]
                            subp_use['simulation_year'] = year_count
                            subp_use['weight'] = weight[month-1]
                            
                        
                        elif weather_mode == 'Synchronized':
                            timestamp = np.datetime64(str(weather_years[k])+'-01-01') + (hour_of_year-1)*np.timedelta64(1,'h')
                            month = timestamp.astype('datetime64[M]').astype(int) % 12 + 1
...
if weather_mode == 'MonteCarlo':
    cols = ['iteration','subproblem','simulation_day','simulation_year','hydro_year','month','weight']
elif weather_mode == 'Synchronized':
    cols = ['iteration','subproblem','simulation_day','simulation_year','hydro_year','weather_date','weather_year','month','weight']
...
for sim_day in day_list:
    ...
    use_day.loc[sim_day,'max_unserved_energy_mw'] = np.max(use_day_tmp['total_unserved_energy'])
    use_day.loc[sim_day,'total_unserved_energy_mwh'] = np.sum(use_day_tmp['total_unserved_energy'])
    use_day.loc[sim_day,'duration_hrs'] = np.sum(use_day_tmp['total_unserved_energy']>0)

use_day.to_csv(os.path.join(os.path.join('Results',case_name,'loss_of_load_days.csv')),index=False)
...
with open(os.path.join('Results',case_name,'summary_metrics.csv'),'w',newline='') as csvfile:
...
for month in range(1,13):
    for HE in range(1,25):
        ...
        EUE_mo_hr.loc[HE,month] = np.sum(use.loc[mask,'total_unserved_energy']*use.loc[mask,'weight'])/N_years
LOLH_mo_hr.to_csv(os.path.join(os.path.join('Results',case_name,'month_hour_LOLH.csv')))
...
if weather_mode == 'Synchronized':
    ...
    for hydro_year in hydro_years:
        for weather_year in weather_years:
            ...
            hourly_mask = (use['hydro_year'] == hydro_year)*(use['weather_year'] == weather_year)
            daily_mask = (use_day['hydro_year'] == hydro_year)*(use_day['weather_year'] == weather_year)
            
            LOLH_hydroweather.loc[hydro_year,weather_year] = np.sum(use.loc[hourly_mask,'weight'])/iterations
            EUE_hydroweather.loc[hydro_year,weather_year] = np.sum(use.loc[hourly_mask,'total_unserved_energy']*use.loc[hourly_mask,'weight'])/iterations
            LOLE_hydroweather.loc[hydro_year,weather_year] = np.sum(use_day.loc[daily_mask,'weight'])/iterations
            LOLP_year_hydroweather.loc[hydro_year,weather_year] = len(np.unique(use.loc[hourly_mask,'simulation_year']))/iterations
    LOLH_hydroweather.to_csv(os.path.join(os.path.join('Results',case_name,'hydro_weather_LOLH.csv')))
    EUE_hydroweather.to_csv(os.path.join(os.path.join('Results',case_name,'hydro_weather_EUE.csv')))
    LOLE_hydroweather.to_csv(os.path.join(os.path.join('Results',case_name,'hydro_weather_LOLE.csv')))
    LOLP_year_hydroweather.to_csv(os.path.join(os.path.join('Results',case_name,'hydro_weather_LOLP.csv')))
 

print('Calculating convergence metrics...')
convergence = pd.DataFrame(0,index = np.arange(iterations)+1,columns = ['LOLH (hrs/yr)','EUE (MWh/yr)','LOLE (days/yr)','LOLP_year (% of years)'])
for i in range(iterations):
    ...
    convergence.loc[i+1,'LOLH (hrs/yr)'] = np.sum(use.loc[hourly_mask,'weight'])/N_years_tmp
    convergence.loc[i+1,'EUE (MWh/yr)'] = np.sum(use.loc[hourly_mask,'total_unserved_energy']*use.loc[hourly_mask,'weight'])/N_years_tmp
    convergence.loc[i+1,'LOLE (days/yr)'] = np.sum(use_day.loc[daily_mask,'weight'])/N_years_tmp
    convergence.loc[i+1,'LOLP_year (% of years)'] = len(np.unique(use.loc[hourly_mask,'simulation_year']))/N_years_tmp
