...
def add_or_update_scenario(db_path, msg):
    ...
    if scenario_exists is not None:
        ...
        update_dict = {
            ...
            'of_fuels': 1 if msg['featureFuels'] else 0,
            'of_multi_stage': 'NULL',
            'of_transmission': 1 if msg['featureTransmission'] else 0,
            'of_transmission_hurdle_rates':
                1 if msg['featureTransmissionHurdleRates'] else 0,
            'of_simultaneous_flow_limits':
                1 if ['featureSimFlowLimits'] else 0,
            'of_lf_reserves_up': 1 if msg['featureLFUp'] else 0,
            'of_lf_reserves_down': 1 if msg['featureLFDown'] else 0,
            'of_regulation_up': 1 if msg['featureRegUp'] else 0,
            'of_regulation_down': 1 if msg['featureRegDown'] else 0,
            'of_frequency_response': 1 if msg['featureFreqResp'] else 0,
            'of_spinning_reserves': 1 if msg['featureSpin'] else 0,
            'of_rps': 1 if msg['featureRPS'] else 0,
            'of_carbon_cap': 1 if msg['featureCarbonCap'] else 0,
            'of_track_carbon_imports':
                1 if msg['featureTrackCarbonImports'] else 0,
            'of_prm': 1 if msg['featurePRM'] else 0,
            'of_local_capacity': 1 if msg['featureELCCSurface'] else 0,
            'of_elcc_surface': 1 if msg['featureLocalCapacity'] else 0,
        ...
        create_scenario(
            ...
            of_transmission_hurdle_rates=1 if msg[
                'featureTransmissionHurdleRates'] else 0,
            of_simultaneous_flow_limits=1 if ['featureSimFlowLimits'] else 0,
            of_lf_reserves_up=1 if msg['featureLFUp'] else 0,
            of_lf_reserves_down=1 if msg['featureLFDown'] else 0,
            of_regulation_up=1 if msg['featureRegUp'] else 0,
            of_regulation_down=1 if msg['featureRegDown'] else 0,
            ...
            of_rps=1 if msg['featureRPS'] else 0,
            of_carbon_cap=1 if msg['featureCarbonCap'] else 0,
            of_track_carbon_imports=1 if msg['featureTrackCarbonImports'] else 0,
            of_prm=1 if msg['featurePRM'] else 0,
            of_local_capacity=1 if msg['featureELCCSurface'] else 0,
            of_elcc_surface=1 if msg['featureLocalCapacity'] else 0,
            temporal_scenario_id=get_setting_option_id(
