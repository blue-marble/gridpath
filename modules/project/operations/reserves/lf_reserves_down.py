...
def add_model_components(m, d, scenario_directory, horizon, stage):
    ...
    m.LF_RESERVES_DOWN_PROJECTS = Set(within=m.PROJECTS)
    m.lf_reserves_down_zone = Param(m.LF_RESERVES_DOWN_PROJECTS)

    m.LF_RESERVES_DOWN_PROJECT_OPERATIONAL_TIMEPOINTS = \
        Set(dimen=2,
            rule=lambda mod:
            set((g, tmp) for (g, tmp) in mod.PROJECT_OPERATIONAL_TIMEPOINTS
                if g in mod.LF_RESERVES_DOWN_PROJECTS))
...
def load_model_data(m, data_portal, scenario_directory, horizon, stage):
    ...
    data_portal.load(filename=os.path.join(scenario_directory,
                                           "inputs", "projects.tab"),
                     select=("project", "lf_reserves_down_zone"),
                     param=(m.lf_reserves_down_zone,)
                     )

    data_portal.data()['LF_RESERVES_DOWN_PROJECTS'] = {
...
def export_module_specific_results(m, d):
    ...
    lf_reserves_down_df = make_project_time_var_df(
        m,
