...
NAME_OF_MODULE_BEING_TESTED = "project.operations.reserves.regulation_down"
...
class TestRegulationDownProvision(unittest.TestCase):
    ...
    def test_data_loaded_correctly(self):
        ...
        actual_projects = sorted([prj for prj in instance.REGULATION_DOWN_PROJECTS])
        self.assertListEqual(expected_projects, actual_projects)

        # Param: regulation_down_zone
        ...
        actual_reserves_zone = OrderedDict(
            sorted(
                {
                    prj: instance.regulation_down_zone[prj]
        ...
        actual_derate = OrderedDict(
            sorted(
                {
                    prj: instance.regulation_down_derate[prj]
                    for prj in instance.REGULATION_DOWN_PROJECTS
                }.items()
            )
        )
        ...
        # Param: regulation_down_reserve_to_energy_adjustment
        # (defaults to 0 if not specified)
        expected_adjustment = OrderedDict(sorted({"Zone1": 0.5, "Zone2": 0.5}.items()))
        actual_adjustment = OrderedDict(
            sorted(
                {
                    z: instance.regulation_down_reserve_to_energy_adjustment[z]
