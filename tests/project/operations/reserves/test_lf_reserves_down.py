...
class TestLFReservesDownProvision(unittest.TestCase):
    ...
    def test_data_loaded_correctly(self):
        ...
        # Set: LF_RESERVES_DOWN_PROJECT_OPERATIONAL_TIMEPOINTS
        ...
        expected_rr_limit = OrderedDict(sorted(
            {"Battery": 1, "Battery_Specified": 1, "Gas_CCGT": 0.1,
             "Gas_CCGT_New": 0.2, "Gas_CCGT_z2": 0.1, "Hydro": 0.2}.items()
        )
        )
        ...
        expected_adjustment = OrderedDict(sorted(
            {"Zone1": .1, "Zone2": 0}.items()
        )
        )
        actual_adjustment = OrderedDict(sorted(
            {z: instance.
                lf_reserves_down_provision_subhourly_energy_adjustment[z]
