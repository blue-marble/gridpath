...
NAME_OF_MODULE_BEING_TESTED = "project.capacity.capacity_types.gen_new_bin"
...
class TestGenNewBin(unittest.TestCase):
    ...
    def test_data_loaded_correctly(self):
        ...
        actual_cost = OrderedDict(
            sorted(
                {
                    (prj, vintage): instance.gen_new_bin_annualized_real_cost_per_mw_yr[
                        prj, vintage
                    ]
                    for (prj, vintage) in instance.GEN_NEW_BIN_VNTS
                }.items()
    ...
    def test_derived_data(self):
        ...
        actual_periods_by_gen_vintage = {
            (prj, vintage): [
                period
                for period in instance.OPR_PRDS_BY_GEN_NEW_BIN_VINTAGE[prj, vintage]
            ]
        ...
        actual_gen_vintage_op_in_period = {
            p: [(g, v) for (g, v) in sorted(instance.GEN_NEW_BIN_VNTS_OPR_IN_PERIOD[p])]
