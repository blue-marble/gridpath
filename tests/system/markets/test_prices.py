...
NAME_OF_MODULE_BEING_TESTED = "system.markets.prices"
...
class TestMarketPrices(unittest.TestCase):
    ...
    def test_initialized_components(self):
        ...
        expected_price = OrderedDict(
            sorted(
                market_price_df.set_index(["market", "timepoint"])
                .to_dict()["price"]
                .items()
            )
        )
        actual_price = OrderedDict(
            sorted(
                {
                    (mrkt, tmp): instance.market_price[mrkt, tmp]
