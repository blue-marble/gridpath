...
class TestFreqRespAgg(unittest.TestCase):
    ...
    def test_data_loaded_correctly(self):
            ...
            sorted(
                {
                    tmp: sorted(
                        [
                            prj
                            for prj in instance.FREQUENCY_RESPONSE_PROJECTS_OPERATIONAL_IN_TIMEPOINT[
                                tmp
                            ]
                        ]
                    )
        ...
        pprojects_2020 = sorted(
            ["Gas_CCGT", "Gas_CCGT_New", "Gas_CCGT_New_Binary", "Gas_CCGT_z2"]
        )
        pprojects_2030 = sorted(
            ["Gas_CCGT", "Gas_CCGT_New", "Gas_CCGT_New_Binary", "Gas_CCGT_z2"]
        )
        expected_pprojects = OrderedDict(
            sorted(
                {
        ...
        actual_pprojects = OrderedDict(
            sorted(
                {
                    tmp: sorted(
                        [
                            prj
                            for prj in instance.FREQUENCY_RESPONSE_PARTIAL_PROJECTS_OPERATIONAL_IN_TIMEPOINT[
                                tmp
                            ]
                        ]
                    )
