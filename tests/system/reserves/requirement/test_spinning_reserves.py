...
NAME_OF_MODULE_BEING_TESTED = "system.reserves.requirement.spinning_reserves"
...
class TestSpinningReservesReq(unittest.TestCase):
    ...
    def test_data_loaded_correctly(self):
        ...
        req_df = pd.read_csv(
            os.path.join(
                TEST_DATA_DIRECTORY, "inputs", "spinning_reserves_tmp_requirement.tab"
            ),
            sep="\t",
        )
        ...
        actual_ba_tmps = sorted(
            [(z, tmp) for (z, tmp) in instance.SPINNING_RESERVES_ZONES * instance.TMPS]
        )
