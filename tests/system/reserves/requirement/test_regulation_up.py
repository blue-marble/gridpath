...
NAME_OF_MODULE_BEING_TESTED = "system.reserves.requirement.regulation_up"
...
class TestCosts(unittest.TestCase):
    ...
    def test_data_loaded_correctly(self):
        ...
        # Set: REGULATION_UP_ZONE_TIMEPOINTS
        req_df = pd.read_csv(
            os.path.join(
                TEST_DATA_DIRECTORY, "inputs", "regulation_up_tmp_requirement.tab"
            ),
            sep="\t",
        )
        ...
        actual_ba_tmps = sorted(
            [(z, tmp) for (z, tmp) in instance.REGULATION_UP_ZONES * instance.TMPS]
        )
            ...
            sorted(
                {
                    ...
                    for (z, tmp) in instance.REGULATION_UP_ZONES * instance.TMPS
                }.items()
            )
        )
