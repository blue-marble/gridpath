...
class TestCarbonTax(unittest.TestCase):
    ...
    def test_data_loaded_correctly(self):
        ...
        expected_ct_zone_periods = sorted([
            ("Carbon_Tax_Zone1", 2020), ("Carbon_Tax_Zone1", 2030),
            ("Carbon_Tax_Zone2", 2020), ("Carbon_Tax_Zone2", 2030)
        ])
        actual_ct_zone_periods = sorted([
            (z, p) for (z, p)
            in instance.CARBON_TAX_ZONE_PERIODS_WITH_CARBON_TAX
        ])
        self.assertListEqual(expected_ct_zone_periods,
                             actual_ct_zone_periods)

        # Param: carbon_tax
        expected_carbon_tax = OrderedDict(sorted({
            ("Carbon_Tax_Zone1", 2020): 30, ("Carbon_Tax_Zone1", 2030): 50,
            ("Carbon_Tax_Zone2", 2020): 10, ("Carbon_Tax_Zone2", 2030): 20}.items()
                                                 )
                                          )
        actual_carbon_tax = OrderedDict(sorted({
            (z, p): instance.carbon_tax[z, p]
            for (z, p) in instance.CARBON_TAX_ZONE_PERIODS_WITH_CARBON_TAX}.items()
                                               )
                                        )
