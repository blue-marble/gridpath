...
NAME_OF_MODULE_BEING_TESTED = "system.policy.carbon_cap.carbon_cap"
...
class TestCarbonCap(unittest.TestCase):
    ...
    def test_data_loaded_correctly(self):
        ...
        expected_cc_zone_periods = sorted([
            ("Carbon_Cap_Zone1", 2020), ("Carbon_Cap_Zone1", 2030),
            ("Carbon_Cap_Zone2", 2020), ("Carbon_Cap_Zone2", 2030)
        ])
        actual_cc_zone_periods = sorted([
            (z, p) for (z, p)
            in instance.CARBON_CAP_ZONE_PERIODS_WITH_CARBON_CAP
        ])
