...
NAME_OF_MODULE_BEING_TESTED = "geography.inertia_reserves_balancing_areas"
...
class TestSpinningReservesBAs(unittest.TestCase):
    ...
    def test_spinning_reserves_zones_data_loads_correctly(self):
        ...
        actual = sorted([z for z in instance.INERTIA_RESERVES_ZONES])
        ...
        actual_allow_violation = OrderedDict(
            sorted(
                {
                    z: instance.inertia_reserves_allow_violation[z]
                    for z in instance.INERTIA_RESERVES_ZONES
                }.items()
            )
        )
        ...
        expected_penalty = OrderedDict(
            sorted({"Zone1": 999999999, "Zone2": 999999999}.items())
        )
        actual_penalty = OrderedDict(
            sorted(
                {
                    z: instance.inertia_reserves_violation_penalty_per_mws[z]
                    for z in instance.INERTIA_RESERVES_ZONES
                }.items()
            )
        )
