...
def validate_inputs(subscenarios, subproblem, stage, conn):
    ...
    bas_w_project = df["lf_reserves_up_ba"].unique()
    ...
    c = conn.cursor()
    bas = c.execute(
        """SELECT lf_reserves_up_ba FROM inputs_geography_lf_reserves_up_bas
    ...
    write_validation_to_database(
        ...
        db_table="inputs_project_lf_reserves_up_bas",
        severity="High",
        errors=validate_idxs(actual_idxs=bas_w_project,
                             req_idxs=bas,
                             idx_label="lf_reserves_up_ba",
...
def write_model_inputs(scenario_directory, subscenarios, subproblem, stage, conn):
    ...
    with open(os.path.join(scenario_directory, str(subproblem), str(stage), "inputs", "projects.tab"), "r"
        ...
        header.append("lf_reserves_up_derate")
