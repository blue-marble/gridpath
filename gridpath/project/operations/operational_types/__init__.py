...
def import_results_into_database(scenario_id, c, db, results_directory):
    ...
    c.execute(
        """DELETE FROM results_project_dispatch_variable WHERE scenario_id = {};""".format(
            scenario_id
        )
    )
    db.commit()
    ...
    c.execute(
        """CREATE TABLE temp_results_project_dispatch_variable""" + str(scenario_id) + """(
        scenario_id INTEGER,
        project VARCHAR(64),
        period INTEGER,
        horizon INTEGER,
    ...
    with open(os.path.join(results_directory, "dispatch_variable.csv"), "r") as \
            v_dispatch_file:
    ...
    # Create temporary table, which we'll use to sort results and then drop
    c.execute(
        """DROP TABLE IF EXISTS temp_results_project_dispatch_capacity_commit"""
        + str(scenario_id) + """;"""
    )
    db.commit()
