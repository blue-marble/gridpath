...
def add_module_specific_components(m, d):
    ...
    *DISPATCHABLE_BINARY_COMMIT_GENERATOR_OPERATIONAL_TIMEPOINTS*.
    ...
    def min_up_time_constraint_rule(mod, g, tmp):
        ...
        # this constraint is redundant (it will simplify to mod.Commit_Binary
        # >= mod.Start_Binary).
    ...
    def min_down_time_constraint_rule(mod, g, tmp):
        ...
        # If only the current timepoint is determined to be relevant,
        # this constraint is redundant (it will simplify to 1-mod.Commit_Binary
        # >= mod.Stop_Binary).
        # This also takes care of the first timepoint in a linear horizon
        # setting, which has only *tmp* in the list of relevant timepoints
        if relevant_tmps == [tmp]:
...
def import_module_specific_results_to_database(scenario_id, c, db,
    ...
    c.execute(
        ...
        temp_results_project_dispatch_binary_commit"""
        + str(scenario_id) + """;"""
    )
    db.commit()

    c.execute(
        """CREATE TABLE temp_results_project_dispatch_binary_commit"""
        + str(scenario_id) + """(
            scenario_id INTEGER,
            project VARCHAR(64),
            period INTEGER,
    ...
    c.execute(
        """DROP TABLE temp_results_project_dispatch_binary_commit""" + str(
            scenario_id) +
        """;"""
    )
    db.commit()
