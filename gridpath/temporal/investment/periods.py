...
# (There is a bit more to it if you are interested: GridPath allows for periods of any duration and not all have to be the same, e.g. some investment decisions can be monthly and some every 10 years; GridPath also allows for timepoints to be of any duration and not all the same, e.g. some are hours, some are 15 minutes, etc. Investment costs inputs are always annualized, however, so we need to properly track all those payment streams and relative weighting with the operational costs through time. The hours_in_period_timepoints is part of the solution. The user-defined temporal structure should actually be such so that in each period, regardless of period param values, the total number of hours (timepoint durations x timepoint weights) and excluding spinup and lookahead timepoints should actually be the number of hours in a year (8760, 8766, or 8784) to ensure consistent weighting of timepoint-level and period-level costs (again, because investment cost inputs are annualized). We don't have investment variables, however, so this doesn't matter for us, as this param just gets multiplied by 0. This functionality wasn't fully developed when we started the project, and we never adjusted the timepoint weights to conform to this -- and I just realized that while I implemented validation for it, I did not put a constraint on the value that makes it into the model even though the documentation says there is one. All of this to say that you can put any number for hours_in_period_timepoints for now -- it doesn't matter.)
