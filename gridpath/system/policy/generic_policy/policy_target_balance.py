...
def add_model_components(
    ...
    def violation_expression_rule(mod, policy, zone, bt, h):
        if mod.policy_zone_allow_violation[policy, zone]:
            return mod.Policy_Target_Shortage[policy, zone, bt, h]
        else:
            return 0

    m.Policy_Target_Shortage_Expression = Expression(
    ...
    m.Policy_Target_Constraint = Constraint(
...
def export_results(
    ...
    #             )
    #         ),
    #         value(m.Policy_Target_Shortage_Expression[z, bt, h]),
    #         (
    #             duals_wrapper(
