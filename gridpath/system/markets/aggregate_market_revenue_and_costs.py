...
def add_model_components(m, d, scenario_directory, subproblem, stage):
    ...
    def total_market_revenue_rule(mod, hub, tmp):
        sum(mod.Sell_Power[prj, tmp]
            for prj in mod.MARKET_HUB_PRJS_OPRTNL_IN_TMP[tmp]
            if mod.market_hub[prj] == hub
            )

    m.Total_Market_Revenue = Expression(
        m.MARKET_HUBS, m.TMPS,
        initialize=total_market_revenue_rule
    )

    def total_market_cost_rule(mod, hub, tmp):
        sum(mod.Buy_Power[prj, tmp]
            for prj in mod.MARKET_HUB_PRJS_OPRTNL_IN_TMP[tmp]
            if mod.market_hub[prj] == hub
            )

    m.Total_Market_Cost = Expression(
        m.MARKET_HUBS, m.TMPS,
        initialize=total_market_cost_rule
    )
