...
def export_results(scenario_directory, subproblem, stage, m, d):
    ...
    with open(
        ...
        writer.writerow(["surface_name", "prm_zone", "period", "elcc_mw"])
        for (s, z, p) in m.ELCC_SURFACE_PRM_ZONE_PERIOD_FACETS:
...
def write_model_inputs(
    ...
    with open(
        ...
        # Writer header
        writer.writerow(
            ["surface_name", "prm_zone", "period", "facet", "elcc_surface_intercept"]
        )
        # Write data
...
def import_results_into_database(
    ...
    results = []
    with open(
        os.path.join(results_directory, "total_contribution_from_elcc_surfaces_mw.csv"),
        "r",
    ) as surface_file:
